...
% %read dftk_proc_data
% fprintf('Reading Proc Data\n')
% for proc = 1:header.header_data.total_processes
%     header.proc_data{proc} = read_proc_data(fid);
% end

align_file(fid);
%read the rest of the header as byte array (print file position first)
% fprintf('proc_data pos: %d\n', ftell(fid));
tail = fread(fid, inf, 'uint8=>uint8');
%remove last 8 bytes (header offset)
header.header_tail = tail(1:end-8);
