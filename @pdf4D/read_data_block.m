...
fseek(fid, time_slice_size * lat2skip, 'bof');
data_block = fread(fid, [total_chans, lat2read], data_format);

%select (and sort) channels
% data(:,:) = data_block(chanindx,:);
%on Magnes I & II channel index in the data block is not the same as in
%channel_data cell array - need conversion
data_index = zeros(size(chanindx));
for chi = 1:length(chanindx)
    %convert from 0-based (c/c++) to 1-based (MATLAB) index
    data_index(chi) = header.channel_data{chanindx(chi)}.index + 1;
end
% data_block = data_block(chanindx,:);
data_block = data_block(data_index,:);
